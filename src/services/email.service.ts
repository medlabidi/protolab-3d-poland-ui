...
export class EmailService {
  ...
  async sendSubmissionConfirmation(toEmail: string, userName: string): Promise<void> {
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>If you have any questions, feel free to contact us at ${EMAIL_USER}</p>
            </div>
            <div class="footer">
              <p>ProtoLab 3D Poland - Professional 3D Printing Services</p>
              <p>&copy; ${new Date().getFullYear()} ProtoLab 3D Poland. All rights reserved.</p>
    ...
    try {
      await transporter.sendMail(mailOptions);
      logger.info(`Submission confirmation sent to ${toEmail}`);
    } catch (error) {
  ...
  async sendAdminNotification(
    ...
    const mailOptions = {
      from: `"ProtoLab Registration System" <${EMAIL_USER}>`,
    ...
    try {
      await transporter.sendMail(mailOptions);
      logger.info(`Admin notification sent for user: ${userEmail}`);
    } catch (error) {
  ...
  async sendApprovalEmail(toEmail: string, userName: string): Promise<void> {
    const mailOptions = {
      ...
      text: `
        ...
        Need help? Contact us at: ${EMAIL_USER}
  ...
  async sendRejectionEmail(toEmail: string, userName: string, reason?: string): Promise<void> {
    const mailOptions = {
      ...
      text: `
        ...
        If you believe this is an error or would like to provide additional information, please contact us at: ${EMAIL_USER}
    ...
    try {
      await transporter.sendMail(mailOptions);
      logger.info(`Rejection email sent to ${toEmail}`);
    } catch (error) {
